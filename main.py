...
def _first_run_setup():
    ...
    console.clear()
    header = Text()
    header.append("  Discord-AI-Server-Generator\n", style="bold cyan")
    header.append("  First-time setup", style="dim")
    console.print(Panel(header, box=box.DOUBLE_EDGE, border_style="cyan", padding=(0, 1)))
    console.print()
    console.print("  No [bold].env[/bold] file found. Let's create one.")
    console.print("  Get your free API key at [underline]https://openrouter.ai/keys[/underline]\n")
    ...
    with open(env_path, "w", encoding="utf-8") as f:
        ...
        f.write("MODEL=openai/gpt-oss-120b:free\n")
    ...
    console.print("\n  [green].env created. Starting...[/green]")
    time.sleep(1)
...
def _validate_startup() -> bool:
    if not OPENROUTER_API_KEY:
        console.print(f"\n  [red]{t('missing_key')}[/red]")
        console.print("  [dim]Add OPENROUTER_API_KEY to your .env file.[/dim]")
        return False

    if not MODEL:
        console.print(f"\n  [red]{t('missing_model')}[/red]")
        console.print("  [dim]Add MODEL to your .env file.[/dim]")
        return False

    with console.status("  [dim]Checking API key and model...[/dim]", spinner="dots"):
        try:
            ...
            if "401" in msg or "auth" in msg.lower() or "api key" in msg.lower():
                ...
                console.print("  [dim]Check OPENROUTER_API_KEY in your .env file.[/dim]")
            elif "404" in msg or "model" in msg.lower():
                console.print(f"\n  [red]Model not found: {MODEL}[/red]")
                console.print("  [dim]Check MODEL in your .env file.[/dim]")
            else:
                console.print(f"\n  [red]API error: {e}[/red]")
            return False
